(function () {
    ...
    function nessInfoProxy(playPanelData, handle, scope) {
        ...
        var chanel = gParams.desSession.swGetPuChanel(channelInfo.puId, channelInfo.index);
    ...
    function CheckAndFullScreen(aimDom) {
        if (aimDom && aimDom.dataDivTag) {
            ...
            if (!fullparentscreen) {
                ...
                chanel.swShow();
                // chanel._updateDialog();
                console.log(aimDomdisitem.currentStyle.width)
            } else {
                // jSW.MP.Frame(divicount);
    ...
    function fillPlayDivShown(focusEle) {
        ...
        // var iIndex = focusEle.dataDivTag._index
    ...
    function closeAllScreen(bContinueDialog) {
        ...
        if (bCloseAll) {
            for (var dialogIndex in gParams.szDialog) {
                closeAChanelByUserData(gParams.szDialog[dialogIndex]);
            }
        }
    }
    ...
    function openChannel(playDivData) {
        ...
        if (chanel) {
            ...
            var rc = chanel.swOpenEx({
                div: div,
                media: jSW.MEDIADIR.VIDEORECV | iAudio,
    ...
    function onOpenChanelOk(playDivData) {
        gParams.szDialog.push(playDivData);
        if (playDivData) {
    ...
    function onOpenChanelOkAddHandle(playDivData) {
        // var handleDiv = playDivData.getOptionsDiv();
        var handleClose = createEleAddClassName("img", "pmHandleClose");
        var handleMax = createEleAddClassName("img", "pmHandleMax");

        handleClose.src = gParams.closeImgBase64;
        handleMax.src = gParams.maxImgBase64;
        ...
        // handleDiv.appendChild(handleMax);
        // handleDiv.appendChild(handleClose);
        // handleDiv.style.textAlign = "right";
        // handleDiv.style.lineHight = "20px";
        ...
        if(iRecordTimes > 300 || iRecordTimes <= 0){
            ...
            if(playDivData == null){
                ...
                if(playDivData){
                    ...
                    var playInfo = playDivData.getPlayDivInfo();
                    var chanel = gParams.desSession.swGetPuChanel(playInfo.puid, playInfo.index);
                    if(chanel){
                        var params = {
                            szpath: recordPath,
    ...
    function closeChannel(puid, index) {
        var chanel = gParams.desSession.swGetPuChanel(puid, index);
        if (chanel) {
        ...
        jSW.MP = {
            ...
            Frame: function (iCount) {
                if (judgeFrameCount(iCount)) {
                    ...
                    var iPmCount = elementsInit(gParams.desDiv, iCount);
                    gParams.iLastCount = iPmCount;
			...
			*/
            ...
            GetStatus: function () {
                var playInfos = gPlayDivMgr.getPlayInfo(gParams.iLastCount);
                return playInfos;
            },
